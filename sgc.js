 ...
 */
/* 
 * The general approach is to wrap the Phaser CE game engine in a simpler API. 
...
class Game {
    ...
    constructor() {
        ...
        this._cache = { images: [] }; // stores data needed by Phaser until Phaser is ready for it
 ...
 */
/* 
 * The general approach is to wrap a Phaser sprite in a simpler API. To hide 
 * Phaser's sprite life cycle, lots of sprite configuration information is cached.
 */
class Sprite {
    constructor() {
        ...
        this.animations = [];
        this._cache = {}; // stores data needed by Phaser until Phaser is ready for it
    ...
    defineAnimation(name, firstFrame, lastFrame) {
        ...
        this.animations.push({ name: name, frames: frameArray });
    }

    /**
     * Starts playback of an animation for this sprite.
     * @argument {string} name - The animation's name, previously defined by a call to this sprite's {@linkcode Game.Sprite.defineAnimation defineAnimation} function.
    ...
    playAnimation(animationName, repeat) {
        ...
        for (let i = 0; i < this.animations.length; i++) {
            if (!this._phaserSprite.animations.getAnimation(this.animations[i].name)) {
                this._phaserSprite.animations.add(this.animations[i].name, this.animations[i].frames, game.frameRate);
            }
        }
